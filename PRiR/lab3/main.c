...
void on_usr1(int signal) {
    ...
    int to = 0;
    if(id == numOfChildren - 1){
        to = count;
    } else {
        to = tab_range[id+1];
    }
    for(int i = from; i < to; i++){
        local_sum = local_sum + vector[i];
    }
    tab_sum[id] = local_sum;
...
int main(int argc, char**argv)
{
    ...
    numOfChildren = argc > 4 ? atof(argv[4]) : 7;
    ...
    for(i=0; i<numOfChildren; i++){
        int dist = floor( (i) * count/numOfChildren);
        tab_range[i] = dist;
        tab_sum[i] = 0;
    }
    ...
    double sum_without_parallel = 0;
    clock_t t;
    t = clock();
    sum_without_parallel = sum(vector, n);
    t = clock() - t;
    double time_taken = ((double)t)/CLOCKS_PER_SEC;
	printf("Suma elementów w wektorze = %f\n", sum_without_parallel);    
    printf("Czas elementow liczony jednym procesem = %f\n", time_taken);
    ...
    printf("Suma policzona z dzielona pamiecia: %f\n", final_sum);
    printf("Czas elementow liczony wieloma procesami = %f\n", time_taken);
